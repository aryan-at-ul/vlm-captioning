...
def get_scheduler(optimizer, config, num_training_steps: int):
    ...
    if scheduler_type == "linear":
        ...
        from transformers import get_cosine_schedule_with_warmup
        # The transformers get_cosine_schedule_with_warmup doesn't accept min_lr_ratio
        ...
        # Custom implementation of cosine schedule with min LR
        min_lr_ratio = config.get("min_lr_ratio", 0.1)
...
def get_cosine_schedule_with_min_lr(optimizer, num_warmup_steps, num_training_steps, min_lr_ratio=0.1, last_epoch=-1):
    """
    Create a schedule with a cosine learning rate from a given linear warmup schedule,
    with a minimum learning rate ratio.
    
    Args:
        optimizer: The optimizer for which to schedule the learning rate
        num_warmup_steps: The number of steps for the warmup phase
        num_training_steps: The total number of training steps
        min_lr_ratio: Minimum learning rate ratio (final lr = initial lr * min_lr_ratio)
